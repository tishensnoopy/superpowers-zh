# Strapi 佑森内容 Seed 脚本 实现计划

> **面向 AI 代理的工作者：** 必需子技能：使用 superpowers:subagent-driven-development（推荐）或 superpowers:executing-plans 逐任务实现此计划。步骤使用复选框（`- [ ]`）语法来跟踪进度。

**目标：** 创建 seed-yousen.js 脚本，基于佑森文件夹真实素材填充 Strapi 全部 content-type

**架构：** 单文件 Node.js 脚本，通过 `createStrapi().load()` 启动 Strapi 实例，用 Document Service API (`strapi.documents()`) 写入数据，用 Upload Service (`strapi.upload`) 上传图片。在 Docker 容器内执行，通过 volume 映射读取佑森图片。

**技术栈：** Node.js, Strapi v5 Document Service API, Strapi Upload Service, Docker Compose volume
...
| 文件 | 职责 |
|------|------|
| `backend/scripts/seed-yousen.js` | **创建** — 主 seed 脚本，含 CLI 解析、数据定义、图片上传、实体 seeder、--remove 逻辑 |
| `backend/src/api/news-article/content-types/news-article/schema.json` | **修改** — 添加 `tags` 字符串字段 |
| `docker-compose.yml` | **修改** — backend 服务添加 `./佑森:/data/yousen:ro` volume 映射 |

---

### 任务 1：给 news-article schema 添加 tags 字段

**文件：**
- 修改：`backend/src/api/news-article/content-types/news-article/schema.json`

- [ ] **步骤 1：在 attributes 中添加 tags 字段**

在 `attributes` 对象中 `sortOrder` 字段后添加：
...
```bash
curl -sS "http://localhost:1337/api/news-articles?fields[0]=tags&pagination[pageSize]=1" | python3 -m json.tool
```

预期：返回 JSON 无报错（tags 字段存在，即使还没有数据）

- [ ] **步骤 4：Commit**

```bash
git add backend/src/api/news-article/content-types/news-article/schema.json
git commit -m "feat: add tags field to news-article schema"
```

---

### 任务 2：Docker Compose volume 映射

**文件：**
- 修改：`docker-compose.yml`

- [ ] **步骤 1：在 backend 服务的 volumes 中添加佑森文件夹映射**

找到 `backend` 服务的 `volumes` 部分，添加：

```yaml
- ./佑森:/data/yousen:ro
```

- [ ] **步骤 2：重启 backend 容器使 volume 生效**
...
```bash
docker compose exec backend ls /data/yousen/LOGO/ | head -3
```

预期：列出 LOGO 文件夹内的 png 文件

- [ ] **步骤 4：Commit**

```bash
git add docker-compose.yml
git commit -m "chore: add yousen images volume mapping for seed script"
```

---

### 任务 3：创建 seed-yousen.js 脚本

**文件：**
- 创建：`backend/scripts/seed-yousen.js`

这是主脚本，约 600-700 行。按以下结构编写：

- [ ] **步骤 1：编写脚本头部 — CLI 解析 + 常量 + 图片路径映射**
...
紧跟在图片路径之后，定义所有实体数据为 JS 对象/数组。数据内容来自设计文档。
...
- [ ] **步骤 3：编写 helper 函数 — 图片上传 + 实体查重/创建/更新/删除**
...
- [ ] **步骤 4：编写 entity seeders — settings/navigation/footer/categories/courses**
...
async function seedNavigation(strapi, force, remove) {
  ...
  for (const item of NAVIGATION) {
    if (remove) { await removeEntity(strapi, uid, item.name); continue; }
    // navigation 用 name 作为查重字段（没有 slug）
...
async function seedSpecs(strapi, force, remove) {
  ...
  for (const spec of SPECS) {
    ...
    const existing = await strapi.documents(uid).findFirst({ filters: { code: { $eq: spec.code } } });
    const data = { ...spec, isVisible: true, status: 'published' };
...
- [ ] **步骤 5：编写 entity seeders — campuses/teachers/faqs/news**
...
async function seedTeachers(strapi, force, remove) {
  ...
  for (const teacher of TEACHERS) {
    ...
    const avatarId = await uploadImage(strapi, TEACHER_IMG[teacher.campusSlug.replace('yousen-', '')]);
...
- [ ] **步骤 6：编写 pages seeder — 首页 sections + 关于我们 + 退费政策**
...
```bash
git add backend/scripts/seed-yousen.js
git commit -m "feat: add seed-yousen.js script with real Yousen data

- Bootstrap Strapi approach, runs inside Docker container
- Seeds 10 entity types: settings, navigation, footer, categories,
  courses, campuses, teachers, faqs, news, pages
- Uploads key images (LOGO, campus, teacher, course, news covers)
- Idempotent: slug-based dedup, --force to update, --remove to delete
- CLI: --only=entity1,entity2 to seed specific entities"
```
...
- [ ] **步骤 1：确保 backend 容器正在运行**

```bash
docker compose ps backend
```

预期：backend 状态为 running
...
- [ ] **步骤 3：验证 Strapi API 返回数据**

```bash
# 课程
curl -sS "http://localhost:1337/api/products" | python3 -c "import sys,json; d=json.load(sys.stdin); print(f'课程数: {len(d.get(\"data\",[]))}')"
# 校区
curl -sS "http://localhost:1337/api/campuses" | python3 -c "import sys,json; d=json.load(sys.stdin); print(f'校区数: {len(d.get(\"data\",[]))}')"
# 教师
curl -sS "http://localhost:1337/api/teachers" | python3 -c "import sys,json; d=json.load(sys.stdin); print(f'教师数: {len(d.get(\"data\",[]))}')"
# 新闻
curl -sS "http://localhost:1337/api/news-articles" | python3 -c "import sys,json; d=json.load(sys.stdin); print(f'新闻数: {len(d.get(\"data\",[]))}')"
# FAQ
curl -sS "http://localhost:1337/api/faq-items" | python3 -c "import sys,json; d=json.load(sys.stdin); print(f'FAQ数: {len(d.get(\"data\",[]))}')"
# 首页 sections
curl -sS "http://localhost:1337/api/pages/homepage" | python3 -c "import sys,json; d=json.load(sys.stdin); s=d.get('data',{}).get('sections',[]); print(f'首页sections数: {len(s)}')"
```

预期：课程3、校区6、教师6、新闻10、FAQ8、首页sections7

- [ ] **步骤 4：验证前端页面显示**

```bash
# 首页大小应 > 50KB
curl -sS "http://localhost:3000/" | wc -c
# 各页面返回 200
curl -sS -o /dev/null -w "%{http_code}" "http://localhost:3000/courses"
curl -sS -o /dev/null -w "%{http_code}" "http://localhost:3000/campuses"
curl -sS -o /dev/null -w "%{http_code}" "http://localhost:3000/teachers"
curl -sS -o /dev/null -w "%{http_code}" "http://localhost:3000/news"
curl -sS -o /dev/null -w "%{http_code}" "http://localhost:3000/faq"
```

预期：首页 > 50000 bytes，各页面返回 200

- [ ] **步骤 5：验证幂等性 — 再次运行 seed 脚本**
...
```bash
docker compose exec backend node scripts/seed-yousen.js --only=courses --force
```
