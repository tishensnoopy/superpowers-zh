...
| 任务 | 状态 | Commit | 备注 |
|------|------|--------|------|
| T1 nginx healthcheck | ✅ | 999736c | /healthz 路径已加 |
| T2 backup cron | ✅ | (服务器配置) | root crontab 0 3 * * * |
| T3 swap | ✅ | (服务器配置) | 2G swap 已创建 |
| T4 rbac contentTypesToAllow | ✅ | b003643 | appointment/feedback 禁 delete |
| T5 admins 角色测试 | ✅ | 8385ae0 | |
| T6 密码重置 | ✅ | 2d2c6af | |
| T7 锁定/解锁 | ✅ | 52f0dfe, a0826af | UI 按钮已加 |
| T8 appointment find/findOne | ✅ | d3e63b4 | |
| T9 feedback API | ✅ | 997e41d | |
| T10 stats API | ✅ | 774ef38 | |
| T11 CSV 导出 | ✅ | e5399ed, bcf6217 | |
| T12 knowledge-base 向量化状态 | ✅ | 4889bf6 | vectorizationStatus 字段 |
| T13-T17 业务审查 | ✅ | 547013e | 报告见 BUSINESS-AUDIT-REPORT.md |
| T18 npm audit | ⚠️ | b11379f→64583cb | 修复后回滚，详见 CODE-QUALITY-REPORT.md |
| T19 版本一致性 | ✅ | e64de2a | |
| T20 lint | ✅ | 3a5c613 | |

**额外修复（2026-07-17，不在原计划内）**：15 个 schema.json 的 `kind: "contentType"` → `"collectionType"`（commit 3c1abc6）。此修复是 Content Manager 正常工作的前提。同时修复 i18n locale（en→en-US + 新增 zh-CN + 业务表 locale NULL 填充 + 默认 locale 改 zh-CN）。
...
- [x] **步骤 1：修改 nginx.conf 80 端口 server 块**
...
- [x] **步骤 2：修改 docker-compose.nginx.yml healthcheck**
...
- [x] **步骤 6：验证 central-nginx healthy**
...
- [x] **步骤 1：修改 rbac.ts，补全 contentTypesToAllow**
...
- [x] **步骤 2：修改 configureClientAdminPermissions，对 appointment 和 feedback 禁用 delete**
...
- [x] **步骤 2：启动 central dev server（如果未运行）**
...
- [x] **步骤 6：修改 login 路由，检查 locked 字段**
...
- [x] **步骤 3：修改 appointment 路由，添加 find/findOne**
...
- [x] **步骤 4：修改 appointment controller，添加 find/findOne 方法**
...
- [x] **步骤 4：在 appointment controller 添加 export 方法**
...
- [x] **步骤 1：在 knowledge-base schema 中添加 vectorizationStatus 字段**
...
- [x] **步骤 2：记录 error 和 warning 数量到报告**
