...
日期：2026-07-19
状态：✅ 用户已决策（2026-07-19），进入实施

## 用户决策记录（D1~D8）

- D1：是 —— Q1 按 A~F 全量执行
- D2：不补 —— central 不给客户开自助登录账号/品牌预览
- D3：Editor 面板账号 —— 需专项设计"与 Strapi 后台的联动"：账号管理 + 功能管理，运营方方便、客户也方便、功能完备
- D4：是 —— 《客户运营手册》+ admin UI 全流程冒烟测试
- D5：是 —— bootstrap 自检自愈
- D6：central agent 全自动 —— 部署管道统一走 central agent（含从零开通编排能力）；与 D2 合并解读：部署全自动但不建客户自助账号
- D7：保留现有占位种子 —— 但必须设计好**知识库注入的隔离逻辑**（母站克隆时 KB 不携带母站内容，由新实例自身内容重新生成）
- D8：执行顺序 Q1 → Q5 → Q3/Q4 → Q2
- 另：83 个遗留变更已 commit（e40baed）
