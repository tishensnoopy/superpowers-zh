...
权限隔离是项目硬约束，需要独立的测试场景覆盖 API 边界、前端路由、安全和数据隔离。

#### API 权限边界

| #   | 测试                         | 验证点                                          |
| --- | -------------------------- | -------------------------------------------- |
| G1  | 访客调用 `/api/appointments` find | 403（访客只能 create）                                |
| G2  | 访客调用 `/api/feedbacks` find | 403                                          |
| G3  | client-admin 调用 `/api/appointments` delete | 403（硬约束：不可删除提交数据）                              |
| G4  | client-admin 调用 `/api/feedbacks` delete | 403                                          |
| G5  | client-admin 调用 `/api/users` | 403（不能管理用户）                                  |
| G6  | client-admin 调用 `/admin/settings` | 403（不能访问 Settings）                            |
| G7  | 未认证用户调用 authenticated API  | 401                                          |
| G8  | 无效 token 调用 API            | 401                                          |
| G9  | 过期 token 调用 API            | 401                                          |
| G10 | 5 个 Chat API 无需鉴权可访问      | 200（硬约束：public）                               |

#### 前端路由权限边界

| #   | 测试                          | 验证点                  |
| --- | --------------------------- | -------------------- |
| G11 | 访客直接访问 `/admin`            | 重定向到登录页               |
| G12 | client-admin 访问 Settings 菜单 | 菜单不显示                |
| G13 | client-admin 直接输入 `/admin/settings` URL | 403 或重定向              |
| G14 | Strapi Admin API token 泄露  | token 有权限范围限制         |

#### 安全边界

| #   | 测试                      | 验证点                                |
| --- | ----------------------- | ---------------------------------- |
| G15 | SQL injection 尝试（预约表单）| 被 Strapi 参数化查询拦截                   |
| G16 | XSS 尝试（富文本字段）          | 被 sanitize 拦截                      |
| G17 | 文件上传类型限制                | 非图片/文档被拒绝                           |
| G18 | 文件上传大小限制                | 超限被拒绝                               |
| G19 | API Rate limiting       | AI 客服防滥用（500 字符 + 10 轮 + 0.3 相似度）   |
| G20 | CSRF 防护                | 跨站请求被拒绝                            |
...
| #   | 测试                             | 验证点                                    |
| --- | ------------------------------ | -------------------------------------- |
| G21 | 不同客户的数据隔离                    | 客户 A 的管理员看不到客户 B 的数据（每个客户独立 Strapi 实例）   |
| G22 | 预约数据只能查看不能删除                 | client-admin 无法删除                       |
| G23 | 反馈数据只能查看/更新状态不能删除           | client-admin 无法删除                       |
| G24 | 知识库文档可上传但向量库配置只能 Super Admin 管理 | client-admin 无法改向量库配置                  |
...
| #  | 故障模拟                  | 验证点                                                        |
| -- | --------------------- | ---------------------------------------------------------- |
| I1 | PostgreSQL 断开         | 后端返回 503 而非崩溃；DB 恢复后自动重连                                    |
| I2 | Redis 不可用             | BullMQ 队列暂停；AI 客服降级到无记忆模式；非关键功能继续运行                          |
| I3 | MeiliSearch 不可用       | 搜索功能降级为 Strapi 默认查询；页面其他功能正常                               |
| I4 | 向量库（Qdrant）不可用       | AI 客服降级为无 RAG 的通用回复；提示用户"知识库暂时不可用"                          |
| I5 | Central 不可达           | Agent 本地继续运行；缓存配置继续生效；5min 后重连                              |
| I6 | LLM API（DashScope）不可用 | AI 客服返回"服务暂时不可用，请稍后再试"；不影响其他功能                              |
| I7 | Strapi Admin 登录失败 5 次 | 账号锁定机制（如果有）                                                |
| I8 | Next.js SSG 构建时后端不可达  | 构建失败 + 错误日志 + 不影响已部署版本                                     |

### 4.10 场景 J：性能与压力

| #  | 测试                      | 验证点                              |
| -- | ----------------------- | -------------------------------- |
| J1 | 课程列表 100+ 条             | 分页正常、响应 < 500ms                 |
| J2 | MeiliSearch 搜索响应        | < 100ms                          |
| J3 | 10 个并发用户访问首页           | LCP < 2.5s                       |
| J4 | 首页 20 张图片加载             | lazy load 生效、CLS < 0.1         |
| J5 | Strapi Admin 1000 条预约列表 | 分页正常、筛选 < 1s                    |
| J6 | AI 客服并发 10 个会话          | 无 race condition、响应 < 3s         |
| J7 | 知识库向量化 50 个文档          | 队列正常处理、无内存泄漏                    |
| J8 | 前端 SSG 构建时长             | < 5 分钟（100 页面）                  |

### 4.11 场景 K：国际化边界

| #  | 测试                  | 验证点                                          |
| -- | ------------------- | -------------------------------------------- |
| K1 | 内容只有中文没有英文         | 中文正常显示、英文显示中文 fallback 或占位提示（不崩溃）            |
| K2 | 内容只有英文没有中文         | 同上反向                                         |
| K3 | 长文本（德语/俄语）布局       | 不破坏布局                                        |
| K4 | 日期格式本地化             | 中文"2026年7月16日"、英文"July 16, 2026"             |
| K5 | 货币格式                | 中文"¥100"、英文"100 CNY"或"$100"视配置              |
| K6 | 地址格式                | 中文"北京市朝阳区..."、英文"Chaoyang, Beijing..."      |
| K7 | URL 结构              | /zh/courses/xxx 和 /en-US/courses/xxx 独立可访问   |
| K8 | sitemap hreflang    | 中英文互指正确                                      |
...
| -- | ------------------ | ------------------------- |
| L1 | Chrome（最新版）        | 全功能正常                     |
| L2 | Firefox（最新版）       | 全功能正常                     |
| L3 | Safari（最新版）        | 全功能正常                     |
| L4 | Edge（最新版）          | 全功能正常                     |
| L5 | 微信内置浏览器            | 首页、课程、预约表单、AI 客服正常        |
| L6 | iOS Safari         | 响应式布局正常                   |
| L7 | Android Chrome    | 响应式布局正常                   |
| L8 | 旧版浏览器（IE 降级提示）    | 显示升级提示而非白屏                |

### 4.13 不纳入本次范围的场景（记录到 known-issues）

以下场景因优先级或范围原因不纳入本次审查，记录到 known-issues：

| 场景 | 描述 | 不纳入原因 |
|---|---|---|
| 通知与消息 | 新预约/反馈/转人工时通知客户管理员 | 需要独立设计通知系统（邮件/站内/微信模板消息），超出本次范围 |
| 可访问性（a11y）| 键盘导航、屏幕阅读器、颜色对比度 | 低优先级，后续迭代 |
| 数据生命周期 | 草稿/发布、过期数据、归档、删除策略、PIPL 合规 | 需要独立设计数据治理策略 |
| 媒体资源管理 | 图片存储路径、orphan 清理、CDN | 后续迭代 |
| 运营与维护 | 日志查看、监控告警、备份恢复演练 | 后续迭代 |

### 4.14 UI/UX 测试点（贯穿所有场景）
...
* `docs/BUSINESS-AUDIT-REPORT.md`：端到端业务流程审查报告（按场景 A/B/C/D/E/F/G/I/J/K/L 分章节，附截图和修复建议）
...
* ✅ P2 端到端业务流程审查报告完成（场景 A/B/C/D/E/F/G/I/J/K/L + UI/UX）
...
16. **权限隔离**：场景 G，独立测试 API 边界(10)+前端路由(4)+安全(6)+数据隔离(4)，共 24 项
17. **容灾降级**：场景 I，模拟 8 种组件故障验证降级策略
18. **性能压力**：场景 J，8 项性能指标验证
19. **国际化边界**：场景 K，8 项边界测试（缺翻译/长文本/日期/货币/地址）
20. **浏览器兼容性**：场景 L，8 种浏览器验证（含微信内置浏览器）
21. **不纳入场景**：通知/a11y/数据生命周期/媒体管理/运营维护，记录到 known-issues
