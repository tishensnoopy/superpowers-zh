...
test.describe('首页', () => {
  ...
  // 根因：首页 Strapi 数据未配置 `section.floating-button`，SectionRenderer 不会渲染 FloatingButton 组件，
  // 因此 div.fixed.z-50 容器在首页不存在。待后端在首页 sections 中加入 floating-button section 后，
  // 移除 test.fixme 改回 test 即可恢复验证。
  test.fixme('FloatingButton 可见', async ({ page }) => {
    await page.goto('/');
    await page.waitForLoadState('networkidle');
